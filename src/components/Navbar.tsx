...
const Navbar: React.FC = () => {
  ...
  useEffect(() => {
    if (isAdmin && showAdminLogin) {
      console.log('Navbar: Admin logged in, keeping modal open for a moment');
  ...
  // Debug logging
  useEffect(() => {
    console.log('Navbar: State changed - isAdmin:', isAdmin, 'showAdminLogin:', showAdminLogin);
  }, [isAdmin, showAdminLogin]);
  ...
  return (
    <nav className="bg-black/50 backdrop-blur-md border-b border-white/20 sticky top-0 z-50">
            ...
            {isAdminReady && (
              isAdmin ? (
                ...
                <button
                  ...
                  className="px-4 py-2 bg-gray-600/20 border border-gray-500/30 text-gray-400 rounded-lg hover:bg-red-600/30 transition-colors"
                >
                  Admin
                </button>
              )
              ...
              {isAdminReady && (
                isAdmin ? (
                  ...
                  <button
                    ...
                    className="w-full px-4 py-2 bg-gray-600/20 border border-gray-500/30 text-gray-400 rounded-lg hover:bg-red-600/30 transition-colors"
                  >
                    Admin
                  </button>
                )
      ...
      {showAdminLogin && (
        <AdminLogin 
          onClose={() => {
            console.log('Navbar: AdminLogin modal closing');
            setShowAdminLogin(false);
          }} 
